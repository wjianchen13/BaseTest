...
public class TestImageSpan extends ImageSpan {
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
        ...
        getTextBounds(paint, KiwiiScreenUtils.dip2px(12));
    ...
    public TestImageSpan(Context context, Drawable d, String sid, String badge, int align, int width, int height) {
        super(d, ImageSpan.ALIGN_BASELINE);
        KiwiiScreenUtils.setContext(context);
        xiu_align = align;
        this.sid = sid;
        this.badge = badge;
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        if(xiu_align == KiwiiScreenUtils.KIWII_ALIGN_CENTER) { // 居中对齐
        ...
        // draw level
        getTextBounds(paint, KiwiiScreenUtils.dip2px(10));
        paint.getTextBounds(sid, 0, sid.length(), bounds);
        int textHeight = bounds.bottom - bounds.top;
        int tx = (KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) - bounds.width()) / 2 - KiwiiScreenUtils.dip2px(0.5f);
        int ty = (bottom - textHeight) / 2 + textHeight - bounds.bottom - 1;
        paint.setColor(Color.rgb(255, 255, 255));
        canvas.drawText(sid, tx , ty , paint);

        // draw badge
        getTextBounds(paint, KiwiiScreenUtils.dip2px(12));
        tx = KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) + (width - (KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) + KiwiiScreenUtils.dip2px(INTER_MARGIN_RIGHT) + bounds.width())) / 2;
...
//    @Override
    public void draw1(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        Drawable b = getCachedDrawable();
        int transY = 0;
        paint.setTextSize(KiwiiScreenUtils.dip2px(12));
        Paint.FontMetrics fm = paint.getFontMetrics();
...
//        System.out.println("=================> fm.ascent222: " + fm.ascent + "   fm.top:" + fm.top + "   fm.bottom:" + fm.bottom + "   fm.descent:" + fm.descent);
        ...
        int tx = KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) + (width - (KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) + KiwiiScreenUtils.dip2px(INTER_MARGIN_RIGHT) + bounds.width())) / 2;
...
//        paint.getTextBounds(sid, 0, sid.length(), bounds);
//        int tx = (KiwiiScreenUtils.dip2px(INTER_BADGE_WIDTH) - bounds.width()) / 2 - KiwiiScreenUtils.dip2px(0.5f);
//        int ty = (height + bounds.height()) / 2 - KiwiiScreenUtils.dip2px(0.0f);
//        paint.setColor(Color.rgb(255, 255, 255));
//        canvas.drawText(sid, tx , ty , paint);
//
//        // draw badge
...
//        System.out.println("=================> bounds.left: " + bounds.left + "   bounds.top:" + bounds.top + "   bounds.right:" + bounds.right + "   bounds.bottom:" + bounds.bottom);
//        System.out.println("=================> height: " + height);
//        ty = (height + bounds.height()) / 2 - KiwiiScreenUtils.dip2px(2.0f);
